...
object HipStubs:
  ...
  def stubIdentifierSearchNoMatch(): StubMapping =
    StubMaker.make(
      httpMethod = StubMaker.HttpMethod.POST,
      urlPattern = urlPathEqualTo(identifierSearchUrl),
      responseStatus = 200,
  ...
  def stubIdentifierSearchError(status: Int): StubMapping =
    StubMaker.make(
      httpMethod = StubMaker.HttpMethod.POST,
      urlPattern = urlPathEqualTo(identifierSearchUrl),
