...
class AgentApplicationEncryption @Inject() (fieldLevelEncryption: FieldLevelEncryption):
  ...
  private def transformAgentDetails(
    ...
    .modify(_.agentEmailAddress.each.emailAddress.agentEmailAddress.value).using(cryptoOp)
    .modify(_.agentEmailAddress.each.emailAddress.otherAgentEmailAddress.each.value).using(cryptoOp)
