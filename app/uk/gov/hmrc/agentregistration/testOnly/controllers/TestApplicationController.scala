...
extends BackendController(cc):
  ...
  private def makeSubmittedApplication(): AgentApplication = AgentApplication(
    linkId = LinkId(value = UUID.randomUUID().toString),
    internalUserId = InternalUserId(value = UUID.randomUUID().toString),
    createdAt = Instant.now(),
    applicationState = ApplicationState.Submitted,
    utr = Some(Utr(value = "1234567890")),
    businessDetails = Some(PartnershipDetails(
      safeId = SafeId("safe-id-12345"),
      businessType = Partnership.LimitedLiabilityPartnership,
      companyProfile = Some(CompanyProfile(
        companyNumber = "1234556678",
