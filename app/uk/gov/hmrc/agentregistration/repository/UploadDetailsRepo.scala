...
//  ExecutionContext
//)
//extends PlayMongoRepository[Upload](
//  collectionName = "upscan-upload-details",
//  mongoComponent = mongoComponent,
//  domainFormat = UploadRepo.mongoFormat,
//  indexes = Seq(
...
//):
//
//  import UploadRepo.given
//
//  def insert(details: Upload): Future[Unit] = collection.insertOne(details)
//    .toFuture()
//    .map(_ => ())
//
//  def findByUploadId(uploadId: UploadId): Future[Option[Upload]] = collection.find(equal("uploadId", Codecs.toBson(uploadId))).headOption()
//
