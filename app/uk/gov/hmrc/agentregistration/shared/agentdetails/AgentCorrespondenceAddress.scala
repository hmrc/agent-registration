...
final case class AgentCorrespondenceAddress(
  ...
  def toValueString: String = Seq(
    addressLine1.replaceCommasWithSpaces,
    addressLine2.getOrElse("").replaceCommasWithSpaces,
    addressLine3.getOrElse("").replaceCommasWithSpaces,
    addressLine4.getOrElse("").replaceCommasWithSpaces,
    postalCode.getOrElse("").replaceCommasWithSpaces,
    countryCode
  )
